  ...
  return (
    <div style={{ background: "#fff", width: "100%", aspectRatio: "3/4", fontFamily: FONT_FAMILY, position: "relative", overflow: "hidden", display: "flex", flexDirection: "column" }}>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "36px 32px" }}>
        ...
        <EditableText v={s.heading} on={ed?.heading} block style={{ fontSize: 24, fontWeight: 700, color: "#1a1a1a", lineHeight: 1.3, marginBottom: 16 }} />
        <div style={{ flex: 1 }}>
          <EditableText v={s.text} on={ed?.text} block style={{ fontSize: 15, color: "#555", lineHeight: 1.9 }} />
        </div>
        {s.extra && (
          <div style={{ marginTop: 24, padding: "18px 22px", background: `${a}08`, borderRadius: 8, borderLeft: `4px solid ${a}` }}>
            <EditableText v={s.extra} on={ed?.extra} block style={{ fontSize: 14, color: "#444", lineHeight: 1.75 }} />
          </div>
        )}
